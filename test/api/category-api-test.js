...
import { maggie, pubCategory, testCategories } from "../fixtures.js";
...
suite("Category API tests", () => {
  ...
  test("create category", async () => {
    const returnedCategory = await placemarkService.createCategory(pubCategory);
    assert.isNotNull(returnedCategory);
    assertSubset(pubCategory, returnedCategory);
  });

  test("delete a category", async () => {
    const category = await placemarkService.createCategory(pubCategory);
